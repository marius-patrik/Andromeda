...
- `packages/manager` owns the `agents` CLI, canonical state, installs,
  credentials/secrets, providers, sessions, memory, packages, and lifecycle
  management.
- `packages/core` owns protobuf contracts and generated Go, TypeScript, and
  Python clients.
- `packages/harness` owns operation: orchestration, canonical session events,
  and the event-backed tool loop; it does not own state-root or
