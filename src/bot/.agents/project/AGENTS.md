...
- Keep managed-file discovery in `src/managed-files.ts` and fail closed unless
  the sole `agent-os-data` registration names the repository declared by
  `.agents/managed-repository.json.dataRepo` and resolves exactly to the
  canonical `$ANDROMEDA_HOME` checkout root;
...
- Route local model work through `andromeda`; do not add provider homes, model
...
- DarkFactory Autoreview delegates every model-backed review and fix turn to
  the canonical `andromeda` launcher; workflow policy selects logical tiers, not a
