...
- Shared identity, memory, roles, skills, sessions, providers, and model choices
  are authoritative only under `$ANDROMEDA_HOME`.
...
- The sole `agent-os-data` registration resolves directly to the canonical
  repository declared by `.agents/managed-repository.json.dataRepo` at
  `$ANDROMEDA_HOME`; unrelated data registrations may coexist but cannot claim
  that repository or path.
- Local worker execution uses the canonical `andromeda` launcher and its configured
  defaults. DarkFactory carries no provider registry, failover list, or model pin.
- DarkFactory Autoreview uses logical model tiers through the canonical `andromeda`
