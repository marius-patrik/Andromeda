...
export const HUMAN_COMMANDS: readonly CommandSpec[] = Object.freeze([
  ...
  command({
    ...
    defaults: "Exact target required; equivalent to df setup <owner/repo> for the same target and JSON mode.", model: "Deterministic; zero model tokens. --tier and --effort are rejected.", permissions: "Repository setup PR/workflow and manifest-authorized private-data ledger path.",
  ...
  ...(["review", "fix"] as const).map((verb) => command({
    ...
    permissions: "Issue read/write and manifest-authorized private-data ledger write for the exact managed target.", mutations: "Bounded autofix may update only the selected issue after immediate version revalidation and preserves owner history.",
  ...
  command({
    ...
    defaults: "No inferred target or question.", model: "Deterministic; zero model tokens.", permissions: "Issue comment write and manifest-authorized private-data ledger write.", mutations: "Posts one deduplicated, version-bound owner-question comment; it never guesses or resolves the answer.",
